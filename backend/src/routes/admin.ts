...
// Temporary endpoint to reset database test data safely in production
adminRouter.post("/system/reset-test-data", async (req, res) => {
  try {
    ...
    await writeAuditLog({
      actorId: actor.id,
      action: "PRODUCTION_TEST_DATA_RESET",
      entityType: "System",
      metadata: { initiatedById: actor.id }
    }).catch(() => undefined);

    return res.json({ ok: true, message: "Database test data has been completely reset. All test users, orders, and commissions are wiped." });
  } catch (error) {
